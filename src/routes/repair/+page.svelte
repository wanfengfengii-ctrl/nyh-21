<script lang="ts">
	...
	const restorers = $derived(() => {
		return $users.filter(u => u.role === '修复师' && u.museumId === $currentMuseumId);
	});
	...
	function handleAssign() {
		if (!selectedOrder) return;
		if (!confirm('确认将工单分配给自己？')) return;
		assignRepairOrder(selectedOrder.id, $currentUser?.id || '');
		selectedOrder = $repairOrders.find(r => r.id === selectedOrder?.id) || null;
	}

	function confirmAssign() {
		if (!selectedOrder || !selectedAssigneeId) return;
		const assignee = $users.find(u => u.id === selectedAssigneeId);
		if (!confirm(`确认将工单分配给 ${assignee?.fullName}？`)) return;
		assignRepairOrder(selectedOrder.id, selectedAssigneeId);
		selectedOrder = $repairOrders.find(r => r.id === selectedOrder?.id) || null;
...
<div class="space-y-6">
		...
		<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" on:click={() => (showDetailModal = false)}>
			<div class="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" on:click|stopPropagation>
				...
				<div class="p-6 space-y-6">
					...
					<!-- 操作按钮 -->
					<div class="flex justify-end gap-3 pt-4 border-t border-gray-200 flex-wrap">
						{#if selectedOrder.status === '待分配' && $currentUser?.role === '修复师'}
							<button
	...
	{#if showAssignModal && selectedOrder}
		<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" on:click={() => (showAssignModal = false)}>
			<div class="bg-white rounded-xl shadow-2xl w-full max-w-md" on:click|stopPropagation>
				<div class="p-6 border-b border-gray-200">
					<h3 class="text-xl font-bold text-gray-800">分配修复工单</h3>
					<p class="text-sm text-gray-500 mt-1">{selectedOrder.beaconLightName}</p>
				</div>

				<div class="p-6 space-y-4">
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-2">选择修复师</label>
						<select
							bind:value={selectedAssigneeId}
							class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none"
						>
							<option value="">请选择修复师</option>
							{#each restorers() as restorer}
								<option value={restorer.id}>{restorer.fullName}</option>
							{/each}
						</select>
						{#if restorers().length === 0}
							<p class="text-xs text-gray-500 mt-2">当前馆区暂无修复师</p>
						{/if}
					</div>
				</div>
